<x-layout>
    ...
    <div class="max-w-screen-xl px-4 py-16 mx-auto sm:px-6 lg:px-8">
        <div class="max-w-lg mx-auto">
            ...
            <form class="bg-gray-100 p-8 mt-6 mb-0 space-y-4 rounded-lg shadow-2xl"
                  action="{{route('comments.update',['product' => $comment->product_id, 'comment' => $comment->id])}}"
                  method="POST">
                <h1 class="text-center text-3xl">Edit Comment</h1>
                @csrf
                @method('PUT')
                <br>
                <input value="{{$comment->name}}" type="text" name="name"
