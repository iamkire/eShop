...
<nav class="border-gray-200 px-2 sm:px-4 py-2.5  bg-gray-900">
    <div class="container flex flex-wrap justify-between items-center mx-auto">
        <a href="{{route('welcome')}}" class="flex items-center">
            ...
            <span class="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">Shop Nest</span>
        </a>
            ...
            @if(Auth::check())
                <div class="hidden sm:flex sm:items-center sm:ml-6 px-5">
                    ...
                    @if(Auth::user()->image)
                        ...
                        <img class="w-10 h-7 rounded-full" src="{{asset('../images/' . Auth::user()->image)}}">
                        </a>
                    @endif
                ...
                <button type="button"
                        class="text-white flex mr-3 text-md rounded-full md:mr-0 focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600"
        ...
        @if(Auth::check())
            <button id="dropdownDividerButton" data-dropdown-toggle="dropdownDivider"
                    class="text-base text-white focus:ring-4 focus:outline-none font-medium rounded-lg text-sm px-4 py-2.5 text-center inline-flex items-center"
                    type="button">Sort by category
            ...
            <div id="dropdownDivider"
                 class="p-5 mt-2 hidden z-50 my-4 text-base list-none bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
            ...
            <div class="hidden justify-between items-center w-full md:flex md:w-auto md:order-1" id="mobile-menu-2">
                <ul class=" flex flex-col mt-4 pt-4 pb-2 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium">
                    <li>
                        <a href="{{route('welcome')}}"
                           class=" text-base block py-2 pr-4 pl-3 text-white bg-blue-700 rounded md:bg-transparent md:text-blue-700 md:p-0 dark:text-white"
                    ...
                    <li>
                        @if(Auth::check())
                            <form method="GET" action="{{route('search')}}">
                                @csrf
                                <input type="text" name="search" class="rounded block py-3 pr-4 pl-3 p-5 text-gray-700 border-b border-gray-100   md:border-0  md:p-0 dark:text-gray-400 dark:border-gray-700" placeholder="Search...">
                            </form>
                        @endif
